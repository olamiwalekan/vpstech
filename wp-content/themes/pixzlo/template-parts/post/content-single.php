...
$post_elements = $aps->pixzloThemeOpt( $template.'-items' );
$post_items = array();
$post_items_opt = get_post_meta( get_the_ID(), 'pixzlo_post_items_opt', true );
if( $post_items_opt == '' || $post_items_opt == 'theme-default' ){
	...
	$meta_post_items = get_post_meta( get_the_ID(), 'pixzlo_post_items', true );
		...
		if( $post_items ):
			...
			foreach ( $post_items as $element => $value ) {
				...
				switch($element) {
					...
					case 'title':
						$layout = $aps->pixzloThemeOpt($template.'-post-template');
					?>
						<header class="entry-header">
							<?php echo ( ''. $aps->pixzloPostTitle( $layout ) ); ?>
						</header>
					<?php									
					break;
					...
					?>
						<div class="entry-meta top-meta clearfix">
							<?php $aps->pixzloPostMeta( 'topmeta' ); ?>
						</div>
					<?php
					break;
					...
					case 'thumb':
						if( $layout != 'list-layout' ):
							$post_format = $aps->pixzloPostFormat();
					...
					?>
						<footer class="entry-footer">
							<div class="entry-meta bottom-meta clearfix">
								<?php $aps->pixzloPostMeta( 'bottommeta' ); ?>
							</div>
						</footer>
					<?php
					break;
