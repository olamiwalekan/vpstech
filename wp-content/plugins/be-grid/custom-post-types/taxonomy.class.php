 ...
 *
 */
if( !class_exists( 'Be_Grid_Cuztom_Taxonomy' ) ){
	class Be_Grid_Cuztom_Taxonomy {
		var $name;
		var $title;
		var $plural;
		var $labels;
		...
		function __construct( $name, $post_type_name = null, $args = array(), $labels = array(), $hierarchical ) {
			if( ! empty( $name ) ) {
				...
				if ( $is_reserved_term = Be_Grid_Cuztom::is_reserved_term( $this->name ) ) {
		...
		function register_taxonomy() {
			...
			$labels = array_merge(
				...
				array(
					...
					'search_items' 			=> __( 'Search ' , BE_GRID_CUZTOM_TEXTDOMAIN ),
					'all_items' 			=> __( 'All ' , BE_GRID_CUZTOM_TEXTDOMAIN ),
					'parent_item' 			=> __( 'Parent ' , BE_GRID_CUZTOM_TEXTDOMAIN ),
					'parent_item_colon' 	=> __( 'Parent :', BE_GRID_CUZTOM_TEXTDOMAIN ),
					'edit_item' 			=> __( 'Edit ' , BE_GRID_CUZTOM_TEXTDOMAIN ), 
					'update_item' 			=> __( 'Update ', BE_GRID_CUZTOM_TEXTDOMAIN ),
					'add_new_item' 			=> __( 'Add New ', BE_GRID_CUZTOM_TEXTDOMAIN ),
					'new_item_name' 		=> __( 'New Name', BE_GRID_CUZTOM_TEXTDOMAIN ),
		...
		function add_column( $columns ) {
			...
			$columns['date'] = __( 'Date',BE_GRID_CUZTOM_TEXTDOMAIN );
			return $columns;
		}
		...
		function _post_filter()  {
			...
			if( $typenow == $this->post_type_name )  {
				wp_dropdown_categories( array(
					'show_option_all'	=> sprintf( __( 'Show All %s', BE_GRID_CUZTOM_TEXTDOMAIN ), $this->plural ),
					'taxonomy'       	=> $this->name,
					'name'            	=> $this->name,
					'orderby'         	=> 'name',
 ...
 * @param 	array 			$args
 * @param 	array 			$labels
 * @return 	object 			Be_Grid_Cuztom_Taxonomy
 *
 * @author 	Gijs Jorissen
 * @since 	0.8
 *
 */
if( !function_exists( 'register_be_grid_cuztom_taxonomy' ) ){
	function register_be_grid_cuztom_taxonomy( $name, $post_type_name, $args = array(), $labels = array() )
	{
		$taxonomy = new Be_Grid_Cuztom_Taxonomy( $name, $post_type_name, $args, $labels );
