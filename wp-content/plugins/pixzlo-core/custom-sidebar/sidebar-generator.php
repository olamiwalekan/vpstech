...
class pixzlo_sidebar_generator {

	

	public function pixzlo_sidebar_generator(){

		add_action('widgets_init',array('pixzlo_sidebar_generator','init'), 20);

		add_action('admin_menu',array('pixzlo_sidebar_generator','admin_menu'));

		add_action('admin_enqueue_scripts', array('pixzlo_sidebar_generator','admin_enqueue_scripts'));

		add_action('admin_print_scripts', array('pixzlo_sidebar_generator','admin_print_scripts'));

		if ( current_user_can( 'manage_options' ) ){

			add_action('wp_ajax_add_sidebar', array('pixzlo_sidebar_generator','add_sidebar') );

			add_action('wp_ajax_remove_sidebar', array('pixzlo_sidebar_generator','remove_sidebar') );
		...
		add_action('edit_post', array('pixzlo_sidebar_generator', 'save_form'));

		add_action('publish_post', array('pixzlo_sidebar_generator', 'save_form'));

		add_action('save_post', array('pixzlo_sidebar_generator', 'save_form'));

		add_action('edit_page_form', array('pixzlo_sidebar_generator', 'save_form'));
	...
	public static function init(){
	    ...
	    $sidebars = pixzlo_sidebar_generator::get_sidebars();
	    ...
	    if(is_array($sidebars)){
			...
			foreach($sidebars as $sidebar){
				...
				$sidebar_class = pixzlo_sidebar_generator::name_to_class($sidebar);
	...
	public static function admin_print_scripts(){
		...
		$ajax_nonce = wp_create_nonce( "pixzlo-custom-sidebar" );
	...
	public static function add_sidebar(){
		...
		check_admin_referer( 'pixzlo-custom-sidebar', 'sidebar_generator_nonce' );
		...
		$id = pixzlo_sidebar_generator::name_to_class($name);
		...
		pixzlo_sidebar_generator::update_sidebars($sidebars);
	...
	public static function admin_menu(){
		...
		add_theme_page('Custom Sidebars', 'Custom Sidebars', 'manage_options', 'multiple_sidebars', array('pixzlo_sidebar_generator','admin_page'));
	...
	public static function admin_page(){
		...
		<div class="wrap">
			...
			<h2><?php esc_html_e( 'Sidebar Generator', 'pixzlo-core' ); ?></h2>

			<p>

				<?php esc_html_e( 'The sidebar name is for your use only. It will not be visible to any of your visitors.', 'pixzlo-core' ); ?> 

				<?php esc_html_e( 'A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'pixzlo-core' ); ?>
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr>
					...
					<th><?php esc_html_e( 'Sidebar Name', 'pixzlo-core' ); ?></th>

					<th><?php esc_html_e( 'CSS class', 'pixzlo-core' ); ?></th>

					<th><?php esc_html_e( 'Remove', 'pixzlo-core' ); ?></th>
				...
				<tr class="<?php echo esc_attr( $alt );?>">
					...
					<td><?php echo esc_html( pixzlo_sidebar_generator::name_to_class($sidebar) ); ?></td>
				...
				<?php
					...
					<tr>
						...
						<td colspan="3"><?php esc_html_e( 'No Sidebars defined', 'pixzlo-core' ); ?></td>
	...
	public static function save_form($post_id){
		...
		if(isset($_POST['sbg_edit'])){
			...
			if(!empty($is_saving)){
				...
				add_post_meta($post_id, 'sbg_selected_sidebar', $_POST['pixzlo_sidebar_generator']);

				add_post_meta($post_id, 'sbg_selected_sidebar_replacement', $_POST['pixzlo_sidebar_generator_replacement']);
				...
				<div class="sbg_container">
					...
					//var_dump($wp_registered_sidebars);		
						...
						for($i=0;$i<5;$i++){ ?>
							...
							<select name="pixzlo_sidebar_generator[<?php echo esc_attr( $i );?>]">

								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>>WP Default Sidebar</option>

							<?php

							$sidebars = $wp_registered_sidebars;// pixzlo_sidebar_generator::get_sidebars();
							...
							<select name="pixzlo_sidebar_generator_replacement[<?php echo esc_attr( $i );?>]">
							...
							$sidebar_replacements = $wp_registered_sidebars;//pixzlo_sidebar_generator::get_sidebars();
	...
	*/
	...
	public static function update_sidebars($sidebar_array){
		...
		update_option('pixzlo_custom_sidebars', $sidebar_json);
	...
	*/
	...
	public static function get_sidebars(){
		...
		$sidebar_json = get_option('pixzlo_custom_sidebars');
...
$sbg = new pixzlo_sidebar_generator;
