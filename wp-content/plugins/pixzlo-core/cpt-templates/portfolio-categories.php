...
$cols = $aps->pixzloGetThemeOpt( 'portfolio-grid-cols' );

$gutter = $aps->pixzloGetThemeOpt( 'portfolio-grid-gutter' );

$infinite = $aps->pixzloGetThemeOpt( 'portfolio-infinite-scroll' ) ? 'true' : 'false';

$isotope = $aps->pixzloGetThemeOpt( 'portfolio-grid-type' );

$extra_class .= $aps->pixzloGetThemeOpt( 'portfolio-grid-type' ) == 'normal' ? ' grid-normal' : '';
		...
		<main id="main" class="site-main archive-template <?php echo esc_attr( $extra_class ); ?>" role="main" data-cols="<?php echo esc_attr( $cols ); ?>" data-gutter="<?php echo esc_attr( $gutter ); ?>">
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'post pixzlo-portfolio' ); ?>>
