...
add_action( 'widgets_init', 'pixzlo_zozo_author_load_widget' );

function pixzlo_zozo_author_load_widget() {

	register_widget( 'pixzlo_zozo_author_widget' );

}

class pixzlo_zozo_author_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'zozo_author_widget', 'description' => esc_html__('A widget that displays authors details.', 'pixzlo-core') );
		...
		parent::__construct( 'zozo_author_widget', esc_html__('Pixzlo Author', 'pixzlo-core'), $widget_ops, $control_ops );
					...
					foreach ( $blogusers as $user ) {
						...
						$out .= '<li class="clearfix media">
							...
							<div class="author-meta media-body">
								...
								<p class="author-counts"><span><a href="'. esc_url( $author_link ) .'" rel="bookmark">' . count_user_posts( $user->ID ) .' '. esc_html__('Posts', 'pixzlo-core') .'</a></span><span>'. $comment_count .' '. esc_html__('Comments', 'pixzlo-core') .'</span></p>
	...
	function form( $instance ) {
		...
		<label for="<?php echo esc_attr( $this->get_field_id('listby') ); ?>"><?php esc_html_e('List by:', 'pixzlo-core') ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('listby') ); ?>" name="<?php echo esc_attr( $this->get_field_name('listby') ); ?>" class="widefat categories" style="width:100%;">
			...
			<option value='default' <?php if ('default' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('Default', 'pixzlo-core') ?></option>

			<option value='post' <?php if ('post' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('No.of Post', 'pixzlo-core') ?></option>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'filter_by' ) ); ?>"><?php esc_html_e('Filter by:', 'pixzlo-core'); ?></label>
			...
			<br><small><?php esc_html_e('Filter by - separate by comma(,) eg: administrator,author,editor,subscriber', 'pixzlo-core'); ?></small>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e('Number of author to show:', 'pixzlo-core'); ?></label>
