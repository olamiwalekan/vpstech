...
add_action( 'widgets_init', 'pixzlo_zozo_advance_tab_post_load_widget' );



function pixzlo_zozo_advance_tab_post_load_widget() {

	register_widget( 'pixzlo_zozo_advance_tab_post_widget' );

}



class pixzlo_zozo_advance_tab_post_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'zozo_advance_tab_post_widget', 'description' => esc_html__('A widget that displays your post by category on tab', 'pixzlo-core') );
		...
		parent::__construct( 'zozo_advance_tab_post_widget', esc_html__('Pixzlo Tab Post Widget', 'pixzlo-core'), $widget_ops, $control_ops );
				...
				<div role="tabpanel" class="tab-pane fade show active" id="tab1_<?php echo esc_attr( $widg_id ); ?>">
					...
					<?php 
						...
						if($selpost1 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number1 ? $number1 : 3, 'meta_key' => 'pixzlo_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories1  );	
				...
				<div role="tabpanel" class="tab-pane fade" id="tab2_<?php echo esc_attr( $widg_id ); ?>">
					...
					<?php
						...
						if($selpost2 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number2 ? $number2 : 3, 'meta_key' => 'pixzlo_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories2  );	
				...
				<div role="tabpanel" class="tab-pane fade" id="tab3_<?php echo esc_attr( $widg_id ); ?>">
					...
					<?php 
						...
						if($selpost3 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number3 ? $number3 : 3, 'meta_key' => 'pixzlo_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories3  );	
		...
		$tab1 = '<ul class="post-newsfeed">';										
			...
			if ($loop->have_posts()) {							
				...
				while ($loop->have_posts()) { 
					...
					$tab1 .= '<p class="post-item-comment"><a href="'.get_comments_link(get_the_ID()).'" ><span>'. $comments_count->total_comments .' '. esc_html__( 'Comments', 'pixzlo-core' ) .' </span></a></p><p class="post-item-date"><a href="'.get_day_link( $archive_year, $archive_month, $archive_day).'" ><span>'. get_the_time( get_option('date_format') ) .'</span></a></p>';
	...
	function advanceTabWidgetCommentsOutput($args){
		...
		if ( is_array( $comments ) && $comments ) {
			...
			foreach ( (array) $comments as $comment ) {
				...
				$output .= '<a href="' . esc_url( get_comment_link( $comment ) ) . '">' . get_the_title( $comment->comment_post_ID ) . '</a> #'.  get_comment_excerpt($comment).' '. esc_html__('by', 'pixzlo-core') .' <span class="comment-author-link"><i>' . get_comment_author_link( $comment ) . '</i></span>' ;
	...
	function form( $instance ) {
		...
		<p><h4><u><?php esc_html_e('First Tab Settings:', 'pixzlo-core'); ?></u></h4></p>

		<p>

			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle1' ) ); ?>"><?php esc_html_e('First Tab Title:', 'pixzlo-core'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories1') ); ?>"><?php esc_html_e('Filter by Category:', 'pixzlo-core'); ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('categories1') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories1') ); ?>" class="widefat categories" style="width:100%;">
			...
			<option value='all' <?php if ('all' == $instance['categories1']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'pixzlo-core'); ?></option>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('selpost1') ); ?>"><?php esc_html_e('Select Post Type:', 'pixzlo-core'); ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('selpost1') ); ?>" name="<?php echo esc_attr( $this->get_field_name('selpost1') ); ?>" class="widefat advance_widget_posttab_selpost" style="width:100%;">
			...
			<?php 
				...
				$postviews = array(
					...
					"recent"=> esc_html__( 'Recent Posts', 'pixzlo-core' ),

					"mostlike"=> esc_html__( 'Most Like Posts', 'pixzlo-core' ),

					"mostview"=> esc_html__( 'Most Viewed Posts', 'pixzlo-core' ),

					"comments"=> esc_html__( 'Post Comments', 'pixzlo-core' )
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'number1' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'pixzlo-core'); ?></label>
		...
		<p><h4><u><?php esc_html_e('Second Tab Settings:', 'pixzlo-core'); ?></u></h4></p>

		<p>

			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle2' ) ); ?>"><?php esc_html_e('Second Tab Title:', 'pixzlo-core'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories2') ); ?>"><?php esc_html_e('Filter by Category:', 'pixzlo-core'); ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('categories2') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories2') ); ?>" class="widefat categories" style="width:100%;">
			...
			<option value='all' <?php if ('all' == $instance['categories2']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'pixzlo-core'); ?></option>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('selpost2') ); ?>"><?php esc_html_e('Select Post Type:', 'pixzlo-core'); ?></label> 
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'number2' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'pixzlo-core'); ?></label>
		...
		<p><h4><u><?php esc_html_e('Thrid Tab Settings:', 'pixzlo-core'); ?></u></h4></p>

		<p>

			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle3' ) ); ?>"><?php esc_html_e('Thrid Tab Title:', 'pixzlo-core'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories3') ); ?>"><?php esc_html_e('Filter by Category:', 'pixzlo-core'); ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('categories3') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories3') ); ?>" class="widefat categories" style="width:100%;">
			...
			<option value='all' <?php if ('all' == $instance['categories3']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'pixzlo-core'); ?></option>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('selpost3') ); ?>"><?php esc_html_e('Select Post Type:', 'pixzlo-core'); ?></label> 
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'number3' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'pixzlo-core'); ?></label>
