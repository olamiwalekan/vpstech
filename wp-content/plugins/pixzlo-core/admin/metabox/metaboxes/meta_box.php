...
define( 'PIXZLO_METABOXES_DIR', PIXZLO_CORE_URL . 'admin/metabox/metaboxes' );
...
function pixzlo_meta_box_unique_key() {
...
function pixzlo_post_option_get_sidebars( $name, $selected ){

	$output = '<select name="'. esc_attr( $name ) .'">';

		$output .= '<option value="">'. esc_html__( 'None', 'pixzlo-core' ) .'</option>';
...
function pixzlo_post_option_drag_drop($post_items, $post_items_array, $status) {
...
function pixzlo_post_option_drag_drop_multi( $key, $post_items ) {
...
function pixzlo_post_option_image_select( $field, $values, $default, $title) {
...
function pixzlo_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	$allowed_html = isset( $field['allowed_html'] ) ? $field['allowed_html'] : null;
	...
	switch( $type ) {
		...
		case 'switch':
			...
			$output .= '
				...
				<div class="pixzlo-switch">

					<label class="switch">

						<input type="checkbox" class="pixzlo-switcher" '. ( $def == '1' ? 'checked' : '' ) .'>

						<div class="slider round"></div>

					</label>

					<input type="hidden" class="pixzlo-switcher-stat" name="' . esc_attr( $name ) . '" value="'. esc_attr( $def ) .'" />
			...
			$output .= '<div class="spacing-parent">
  					...
  					$output .= '<input size="5" type="text" name="' . esc_attr( $name .'_0') . '" value="'. esc_attr( $space_values[0] ) .'">
							...
							<select name="' . esc_attr( $name .'_1' ) . '">
								...
								<option value="px" '. ( $space_values['1'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'pixzlo-core' ) .'</option>

								<option value="em" '. ( $space_values['1'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'pixzlo-core' ) .'</option>
						...
						<input type="hidden" name="' . esc_attr( $name ) . '" value="pixzlo_dimension" /><br />' . $desc;
		...
		case 'space':
			...
			$output .= '<div class="spacing-parent">
							...
							<select name="' . esc_attr( $name .'_4' ) . '">
								...
								<option value="px" '. ( $space_values['4'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'pixzlo-core' ) .'</option>

								<option value="em" '. ( $space_values['4'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'pixzlo-core' ) .'</option>
							...
							if( $border_style ){
								...
								$output .= '<select name="' . esc_attr( $name .'_6' ) . '">
									...
									<option value="solid" '. ( $space_values['6'] == 'solid' ? 'selected' : '' ) .'>'. esc_html__( 'Solid', 'pixzlo-core' ) .'</option>

									<option value="dashed" '. ( $space_values['6'] == 'dashed' ? 'selected' : '' ) .'>'. esc_html__( 'Dashed', 'pixzlo-core' ) .'</option>

									<option value="dotted" '. ( $space_values['6'] == 'dotted' ? 'selected' : '' ) .'>'. esc_html__( 'Dotted', 'pixzlo-core' ) .'</option>

									<option value="double" '. ( $space_values['6'] == 'double' ? 'selected' : '' ) .'>'. esc_html__( 'Double', 'pixzlo-core' ) .'</option>

									<option value="none" '. ( $space_values['6'] == 'none' ? 'selected' : '' ) .'>'. esc_html__( 'None', 'pixzlo-core' ) .'</option>
							...
							if( $color_opt ){
								...
								$output .= '<input class="pixzlo-color-picker" id="' . esc_attr( $id.'_5' ) . '" name="' . esc_attr( $name.'_5' ) . '" value="' . esc_attr( $space_values[5] ) . '" type="text" size="10" />';
			...
			$output .= '</div>
						...
						<input type="hidden" name="' . esc_attr( $name ) . '" value="pixzlo_space" /><br />' . $desc;
		...
		case 'link_color':
			...
			$output .= '<div class="link-color-parent">
							...
							<i>'. esc_html__( 'Regular', 'pixzlo-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_0' ) . '" class="pixzlo-color-picker" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $link_values[0] ) . '" size="10"  />

							<i>'. esc_html__( 'Hover', 'pixzlo-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_1' ) . '" class="pixzlo-color-picker" id="' . esc_attr( $id.'_1' ) . '" value="' . esc_attr( $link_values[1] ) . '" size="10"  />

							<i>'. esc_html__( 'Active', 'pixzlo-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_2' ) . '" class="pixzlo-color-picker" id="' . esc_attr( $id.'_2' ) . '" value="' . esc_attr( $link_values[2] ) . '" size="10"  />

						</div>

						<input type="hidden" name="' . esc_attr( $name ) . '" value="pixzlo_link_color" /><br />' . $desc;
		...
		case 'alpha_color':
			...
			$output .= '<div class="pixzlo-alpha-color">';

				$output .= '<input type="text" name="' . esc_attr( $name.'_0' ) . '" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $alpha_values[0] ) . '" class="pixzlo-alpha-color" />

						<input class="pixzlo-color-picker pixzlo-alpha-color-picker" id="' . esc_attr( $id.'_0' ) . '" name="' . esc_attr( $name.'_1' ) . '" value="' . esc_attr( $alpha_values[1] ) . '" type="text" size="10" />

						<input type="range" min="1" max="10" step="1" value="3" class="alpha-range">';

				$output .= '<input type="hidden" name="' . esc_attr( $name ) . '" value="pixzlo_alpha_color" /><br />' . $desc;
		...
		case 'rating':
			...
			$output .= '<ul class="pixzlo-meta-rating star-rating">
						...
						<input type="hidden" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . absint( $meta ) . '" class="pixzlo-meta-rating-value" />
		...
		case 'textarea':
			...
			if( $allowed_html ){
				...
				$output .= '<textarea name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="60" rows="4">' . esc_html( wp_unslash( $meta ) ) . '</textarea><br />' . $desc;

			}else{

				$output .= '<textarea name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="60" rows="4">' . esc_textarea( $meta ) . '</textarea><br />' . $desc;
		...
		case 'color':
			...
			$output .= '<input type="text" name="' . esc_attr( $name ) . '" class="pixzlo-color-picker" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" size="10"  />';
		...
		case 'image':
			...
			$image = PIXZLO_METABOXES_DIR . '/images/image.png';	
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					$output .= pixzlo_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
		...
		case 'dragdrop':
			...
			$t_meta_key = pixzlo_meta_box_unique_key();
			...
			$post_enabled_out = $post_enabled_items != '' ? pixzlo_post_option_drag_drop( $post_enabled_items, $options['items'], 'enabled' ) : '';

			$post_disabled_out =  $post_disabled_items != '' ? pixzlo_post_option_drag_drop( $post_disabled_items, $options['items'], 'disabled' ) : '';

			

			$output .= '<div class="meta-drag-drop-field" data-field="'. $t_meta_key .'">';

				$output .= '<h4>'. esc_html__( 'Enabled Items', 'pixzlo-core' ) .'</h4>';

				$output .= $post_enabled_out;

				$output .= '<h4>'. esc_html__( 'Disabled Items', 'pixzlo-core' ) .'</h4>';
		...
		case 'image_select':
			...
			$output .= pixzlo_post_option_image_select( $name, $options, $selected, $label );
		...
		case 'sidebar':
			...
			$output .= pixzlo_post_option_get_sidebars( $name, $selected);
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( meta_box_find_field_type( 'alpha_color', $this->fields ) ){
				...
				wp_enqueue_style( 'rangeslider', PIXZLO_METABOXES_DIR . '/css/rangeslider.css', array(), '1.0' );

				wp_enqueue_script( 'jquery-rangeslider', PIXZLO_METABOXES_DIR . '/js/rangeslider.min.js', array( 'jquery' ), '1.0', true );

			}

			

			wp_enqueue_script( 'meta_box', PIXZLO_METABOXES_DIR . '/js/scripts.js', $deps );
			...
			wp_register_style( 'jqueryui', PIXZLO_METABOXES_DIR . '/css/jqueryui.css' );
			...
			wp_enqueue_style( 'meta_box', PIXZLO_METABOXES_DIR . '/css/meta_box.css', $deps );
	...
	function meta_box_callback() {
		...
		wp_nonce_field( 'pixzlo_meta_box_nonce_action', 'pixzlo_meta_box_nonce_field' );
		...
		foreach ( $this->fields as $field) {
			...
			if( isset( $field['required'] ) )
				...
				if( $field['type'] != 'label' && $field['type'] != 'line' ){
					...
					$meta_output[$tab] .= pixzlo_meta_box_field( $field, $meta  );
		...
		foreach($meta_output as $key => $value){
			...
			$tab_list .= '<li class="'. ( $i == 0 ? 'current' : '' ) .'"><a href="#pixzlo-admin-tab-'. esc_attr( $tab_id ) .'">'. esc_html( $key ) .'</a></li>';

			$tab_content .= '<div id="pixzlo-admin-tab-'. esc_attr( $tab_id ) .'" class="pixzlo-admin-tab-content'. ( $i == 0 ? ' current' : '' ) .'">';
		...
		<div id="pixzlo-admin-tabs-container">

			<ul class="pixzlo-admin-tabs-menu">

				'. $tab_list .'

			</ul>

			<div class="pixzlo-admin-tab">
	...
	function save_box( $post_id ) {
		...
		if ( ! isset( $_POST['pixzlo_meta_box_nonce_field'] ) )

			return $post_id;

		if ( ! ( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['pixzlo_meta_box_nonce_field'],  'pixzlo_meta_box_nonce_action' ) ) ) 
			...
			elseif( $field['type'] != 'label' && $field['type'] != 'line' ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					if ( is_array( $new ) ){
						...
						if( $field['type'] == 'textarea' && isset( $field['allowed_html'] ) && $field['allowed_html'] ){
